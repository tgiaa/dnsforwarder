...
static struct sockaddr_in *CheckAList(struct sockaddr_in *Ips, int Count) {
  ...
  if (p.Select(&p, &Time, (void **)&Fastest, FALSE, TRUE, NULL) ==
      INVALID_SOCKET) {
    ret = NULL;
  }
...
static int AddToLists(ConfigFileInfo *ConfigInfo) {
  ...
  while ((Itr = sli.Next(&sli)) != NULL) {
    ...
    if (StringChunk_Match_NoWildCard(GoodIpList, n, NULL, (void **)&m, NULL,
                                     NULL) == FALSE) {
...
static int AddTask(void) {
  ...
  while (Domain != NULL) {
    if (m != NULL) {
      TimedTask_Add(TRUE, FALSE, m->Interval, (TaskFunc)ThreadJod,
                    (void *)Domain, (void *)m, TRUE);
    }
...
const char *GoodIpList_Get(const char *List) {
  ListInfo *m;
  if (StringChunk_Match_NoWildCard(GoodIpList, List, NULL, (void **)&m, NULL,
                                   NULL) == TRUE) {
    if (Array_GetUsed(&(m->List)) <= 0) {
      return NULL;
    } else {
      return (const char *)&(
          ((const struct sockaddr_in *)Array_GetBySubscript(&(m->List), 0))
              ->sin_addr);
    }
  } else {
    return NULL;
